...
class RolePermission(Base, TimestampMixin, RoleFKMixin, PermissionFKMixin):
    ...
    id = Column(Integer, primary_key=True, index=True) # 역할-권한 관계의 고유 ID
    allowed_columns = Column(JSON, nullable=True) # 이 권한이 허용하는 컬럼 목록 (JSON 형식)
    filter_condition = Column(JSON, nullable=True) # 이 권한에 적용되는 데이터 필터 조건 (JSON 형식)
    
    # --- 관계 정의 (외래 키) ---
    # role_id는 RoleFKMixin으로부터 상속받습니다.
    # permission_id는 PermissionFKMixin으로부터 상속받습니다.

...
class DeviceComponentInstance(Base, TimestampMixin, DeviceFKMixin, SupportedComponentFKMixin):
    ...
    id = Column(Integer, primary_key=True, index=True) # 장치 컴포넌트 인스턴스의 고유 ID
    # device_id는 DeviceFKMixin으로부터 상속받습니다.
    # supported_component_id는 SupportedComponentFKMixin으로부터 상속받습니다.
    instance_name = Column(String(100), nullable=False) # 장치 내에서 이 컴포넌트 인스턴스의 고유 이름 (예: '메인 온도 센서', '펌프 1')
    description = Column(Text, nullable=True) # 컴포넌트 인스턴스에 대한 설명

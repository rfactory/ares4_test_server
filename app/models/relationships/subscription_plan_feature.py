...
class SubscriptionPlanFeature(Base, TimestampMixin, SubscriptionPlanFKMixin):
    ...
    id = Column(Integer, primary_key=True, index=True) # 구독 플랜 기능의 고유 ID
    # plan_id는 SubscriptionPlanFKMixin으로부터 상속받습니다.
    feature_key = Column(String(100), nullable=False) # 기능의 키 (예: 'max_devices', 'cloud_storage_gb')
    feature_value = Column(Text, nullable=False) # 기능의 값 (예: '10', '50')
    description = Column(Text, nullable=True) # 기능에 대한 설명
    # --- Relationships ---

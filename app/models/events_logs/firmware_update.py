...
class FirmwareUpdate(Base, TimestampMixin, DeviceFKMixin, HardwareBlueprintFKMixin): # Mixin 상속 제거
    ...
    id = Column(Integer, primary_key=True, index=True) # 펌웨어 업데이트 기록의 고유 ID
    # device_id는 DeviceFKMixin으로부터 상속받습니다.
    # hardware_blueprint_id는 HardwareBlueprintFKMixin으로부터 상속받습니다.
    firmware_version = Column(String(50), nullable=False) # 업데이트된 펌웨어의 버전
    update_status = Column(Enum('PENDING', 'IN_PROGRESS', 'COMPLETED', 'FAILED', 'ROLLBACK', name='update_status'), default='PENDING', nullable=False) # 펌웨어 업데이트의 현재 상태 ('PENDING', 'IN_PROGRESS', 'COMPLETED', 'FAILED', 'ROLLBACK')
    initiated_at = Column(DateTime(timezone=True), nullable=False) # 펌웨어 업데이트가 시작된 시간
    completed_at = Column(DateTime(timezone=True), nullable=True) # 펌웨어 업데이트가 완료된 시간
   
    # 명시적으로 외래 키 컬럼 정의
    initiated_by_user_id = Column(Integer, ForeignKey('users.id'), nullable=True) # 펌웨어 업데이트를 시작한 사용자 ID
   
    notes = Column(Text, nullable=True) # 펌웨어 업데이트에 대한 추가 메모

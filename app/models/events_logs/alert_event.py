...
class AlertEvent(Base, TimestampMixin, DeviceFKMixin, AlertRuleFKMixin): # UserFKMixin 제거
    ...
    id = Column(Integer, primary_key=True, index=True) # 알림 이벤트의 고유 ID
    # device_id는 DeviceFKMixin으로부터 상속받습니다.
    # alert_rule_id는 AlertRuleFKMixin으로부터 상속받습니다.
   
    # 명시적으로 외래 키 컬럼 정의
    user_id = Column(Integer, ForeignKey('users.id'), nullable=True) # 이벤트를 발생시킨 사용자 ID
   
    severity = Column(Enum('INFO', 'WARNING', 'CRITICAL', name='alert_severity'), nullable=False) # 알림의 심각도 ('INFO', 'WARNING', 'CRITICAL')
    message = Column(Text, nullable=False) # 알림 메시지 내용
   
    # 명시적으로 외래 키 컬럼 정의
    acknowledged_by_user_id = Column(Integer, ForeignKey('users.id'), nullable=True) # 알림을 확인한 사용자 ID
   
    acknowledged_at = Column(DateTime(timezone=True), nullable=True) # 알림이 확인된 시간

...
class DeviceLog(Base, TimestampMixin, LogBaseMixin, DeviceFKMixin): # Inherit LogBaseMixin
    ...
    event_type = Column(Enum('DEVICE', 'AUDIT', 'CONSUMABLE_USAGE', 'SERVER_MQTT_CERTIFICATE_ISSUED', 'DEVICE_CERTIFICATE_CREATED', 'CERTIFICATE_REVOKED', 'SERVER_CERTIFICATE_ACQUIRED_NEW', name='log_event_type'), nullable=False, default='DEVICE') # 로그 유형 (온톨로지 통합 쿼리 용)

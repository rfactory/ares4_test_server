...
from app.core.exceptions import NotFoundError, PermissionDeniedError # ForbiddenError -> PermissionDeniedError
from app.models.objects.user import User
from app.models.objects.role import Role
from app.models.objects.permission import Permission # Permission 모델 임포트
...
class UpdateRolePermissionsPolicy:
    def execute(
        ...
        *, 
        ...
        """
        # 1. 대상 역할(target_role) 조회 (연결된 조직 정보도 함께 로드)
        target_role = db.query(Role).options(joinedload(Role.organization)).filter(Role.id == target_role_id).first()
        if not target_role:
            raise NotFoundError("Role", str(target_role_id))

        # 2. 규칙: 보호된 역할 (tier 0, 1)의 권한 수정 금지
        ...
        # 4. 규칙: 권한 상속의 부분집합 원칙 (자신이 가지지 않은 권한은 부여할 수 없다)
        # 4a. 요청자의 현재 컨텍스트에서 사용 가능한 모든 권한 이름을 set으로 구성

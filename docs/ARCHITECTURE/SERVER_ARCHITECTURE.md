...
## server2 아키텍처 변경점 (Changes for server2 Architecture)



이 섹션은 `server` 아키텍처를 계승하는 `server2`의 주요 변경 및 추가 사항을 기술합니다. `server2`는 기존 프로토타입을 넘어, 실제 운영 및 B2B SaaS 서비스를 목표로 하는 확장된 아키텍처를 채택합니다.
...
- **(변경) MQTT 브로커 (기존 2, 7, 15절 관련):**

  - 기존 `Mosquitto`에서 `EMQX`로 MQTT 브로커를 변경합니다. EMQX는 대규모 클러스터링 및 높은 처리량을 지원하여 시스템의 확장성을 향상시킵니다.



- **(추가) 인증서 및 비밀 정보 관리: HashiCorp Vault 도입 (기존 7, 8, 9절 관련):**

  - 기존의 `app/core/certificate_manager.py` 및 스크립트 기반 인증서 관리 방식을 폐기합니다.

  - **HashiCorp Vault**를 중앙 집중식 비밀 관리 시스템으로 도입합니다.

    - **PKI Secrets Engine:** 시스템 내 모든 TLS 인증서(EMQX 서버, MQTT 클라이언트 등)의 발급, 갱신, 폐기를 자동화합니다. 단기 유효 인증서(Short-Lived Certificates) 전략을 통해 보안을 강화합니다.

    - **KV Secrets Engine:** HMAC/JWT 시크릿 키, API 키, DB 접속 정보 등 모든 정적 비밀 정보를 안전하게 저장하고, 애플리케이션이 시작 시점에 동적으로 읽어가도록 구성합니다.

  - **인증서 주입 방식:** `EMQX`와 같이 파일 경로로 인증서를 읽어야 하는 애플리케이션의 경우, 컨테이너 시작 시점에 Entrypoint 스크립트가 Vault로부터 인증서를 받아와 컨테이너 내 임시 파일 시스템에 파일로 저장합니다. 이 파일은 컨테이너가 종료될 때 함께 삭제됩니다.



- **(추가) 개발 환경 자동화 (Development Environment Automation):**
  - `server2`의 복잡한 로컬 개발 환경(EMQX, Vault, Postgres, Redis 등) 구성을 단순화하고 표준화하기 위해, `server2/vault/` 디렉토리 내에 3개의 셸 스크립트를 도입합니다.
  - **`dev-init.sh`**: **최초 1회 또는 전체 초기화**를 위해 사용합니다. 기존 Docker 컨테이너, 볼륨, 네트워크를 모두 삭제하고, Vault 컨테이너를 새로 시작하여 인증서 생성까지의 모든 초기 설정을 자동으로 수행합니다. 데이터베이스를 포함한 모든 데이터가 삭제되므로 주의가 필요합니다.
  - **`dev-start.sh`**: **일상적인 개발 세션 시작**을 위해 사용합니다. 데이터 볼륨을 유지한 채 모든 서비스를 `docker-compose up -d --build` 명령으로 시작합니다.
  - **`dev-stop.sh`**: **개발 세션 종료**를 위해 사용합니다. 데이터 볼륨을 유지한 채 `docker-compose down` 명령으로 모든 서비스를 중지합니다.
  - **표준 워크플로우:**
    1. 프로젝트를 처음 설정할 때 `./server2/vault/dev-init.sh`를 한 번 실행합니다.
    2. 개발을 시작할 때 `./server2/vault/dev-start.sh`를 실행합니다.
    3. 개발을 마칠 때 `./server2/vault/dev-stop.sh`를 실행합니다.
    4. 데이터베이스 스키마 변경 등 전체 환경 리셋이 필요할 때만 `dev-init.sh`를 다시 실행합니다.

- **(추가) Docker Compose 환경 변수 관리:**
  - `fastapi_app2`와 같이 컨테이너 시작 시점에 Vault 접근이 필요한 서비스는 `VAULT_TOKEN`과 같은 환경 변수가 필요합니다.
  - **개발 환경:** `docker-compose.v2.yml`의 `environment` 섹션을 통해 `.env` 파일에 정의된 `VAULT_TOKEN`을 컨테이너에 주입합니다. 이는 개발 편의성을 위함입니다.
  - **프로덕션 환경:** 이 방식은 프로덕션에 적합하지 않습니다. 프로덕션 환경에서는 `VAULT_TOKEN`을 직접 사용하는 대신, **Vault AppRole**과 같이 권한이 제한되고 동적으로 생성되는 인증 방식을 사용하여 최소 권한 원칙을 준수해야 합니다.


### 2. 데이터베이스 스키마 주요 변경 사항 (기존 6절 관련)



`server2`는 다중 조직을 지원하는 SaaS 플랫폼으로 발전하면서 데이터베이스 스키마가 대대적으로 확장 및 재구성되었습니다.


- **(추가) 설계 사상: 데이터베이스의 온톨로지화 (Ontological Approach):**
  - `server2`의 스키마는 단순한 데이터 저장을 넘어, 시스템이 다루는 도메인의 **개념(Concepts)과 관계(Relations)를 명시적으로 정의하는 온톨로지적 접근 방식**을 채택합니다. 
  - `hardware_blueprints`, `product_lines`, `organization_types`처럼 추상적인 개념들이 독립된 테이블로 정의되고, 다른 테이블들이 이를 외래 키로 참조함으로써 "A는 B의 한 종류이다", "C는 D로 구성된다"와 같은 의미적 관계가 구조적으로 표현됩니다. 
  - 이를 통해 데이터의 재사용성과 확장성이 극대화되며, 데이터베이스 자체가 시스템의 지식 베이스(Knowledge Base) 역할을 수행하게 됩니다.

- **(추가) 다중 조직(Multi-tenancy) 지원:**

  - `organizations` 및 `organization_types` 테이블이 추가되어, 여러 고객사(조직)를 독립적으로 관리할 수 있는 기반을 마련합니다.

  - `devices`, `roles` 등 대부분의 주요 모델이 `organization_id`를 외래 키로 참조하여 데이터가 조직별로 격리됩니다.



- **(추가) 구독 및 제품 라인 모델:**

  - `subscription_plans`, `product_lines`, `organization_subscriptions` 테이블이 추가되어, 다양한 유/무료 서비스 플랜과 제품 라인을 정의하고 조직별로 구독을 관리할 수 있습니다.



- **(변경) 구조화된 하드웨어 및 부품 관리:**

  - `hardware_blueprints`, `blueprint_pin_mappings` 등 하드웨어의 청사진을 정의하는 모델이 추가되었습니다.

  - `devices` 테이블은 더 이상 `hardware_version`과 같은 정보를 직접 갖지 않고, `hardware_blueprint_id`를 참조하여 하드웨어 구성을 훨씬 더 구조적이고 재사용 가능하게 관리합니다.



- **(변경) 계층적 역할 기반 접근 제어 (RBAC):**

  - `roles` 테이블에 `scope`(SYSTEM/ORGANIZATION), `tier`, `lineage` 컬럼이 추가되어, 시스템 전역 역할과 조직 내 역할, 그리고 역할 간의 상하 관계를 정의하는 등 훨씬 더 세분화되고 강력한 계층적 권한 관리가 가능해졌습니다.



- **(변경) 정규화된 텔레메트리 데이터:**

  - `telemetry_data` 테이블의 `payload` (JSON) 필드가 `metric_name`, `metric_value`, `unit` 등의 정규화된 컬럼으로 변경되어, 데이터 조회 및 분석 성능이 향상되었습니다.



- **(추가) 2단계 인증(2FA) 지원:**

  - `users` 테이블에 `is_two_factor_enabled` 컬럼이 추가되어, 향후 2단계 인증 기능을 구현할 수 있는 기반을 마련했습니다.



- **(변경) 인증서 저장 방식:**

  - `devices` 테이블에서 인증서 관련 필드가 제거되고, 별도의 `certificates` 테이블이 추가되었습니다. 이는 Vault 도입 이전의 과도기적 설계로, Vault 아키텍처에서는 이 테이블 또한 사용되지 않거나 인증서의 메타데이터(예: 시리얼 번호)를 저장하는 용도로 변경될 것입니다。

- **(추가) Vault 운영 및 재해 복구 전략 (Vault Operations and Disaster Recovery Strategy)**

  이 섹션은 `server2` 아키텍처의 핵심 구성 요소인 HashiCorp Vault를 프로덕션 환경에서 안전하고 안정적으로 운영하기 위한 전략과 절차를 기술합니다.

  - **Vault 마스터 키 관리 (Unseal Key & Root Token)**
    - Vault의 보안은 Unseal Key와 Root Token의 안전한 관리에 달려 있으며, 이는 기술이 아닌 **사람과 절차의 영역**입니다.
    - **분산 및 정족수:** Vault 초기화 시, `-key-shares`와 `-key-threshold` 옵션을 사용하여 키를 여러 개로 분리하고, 봉인 해제에 필요한 최소 개수(정족수)를 설정해야 합니다. (예: `5`개의 키 생성, `3`개의 정족수)
    - **독립적인 분산 보관:** 생성된 각 Unseal Key는 **서로 다른 신뢰할 수 있는 담당자**에게 전달되어, **서로 다른 안전한 방식**으로 보관되어야 합니다. (예: 패스워드 관리자, 암호화된 USB, 물리적 금고 등)
    - **1인 관리 시나리오:** 관리자가 한 명일 경우에도, 여러 개의 키를 생성하여 자신만이 접근할 수 있는 서로 다른 물리적/논리적 저장소에 분산 보관하여 단일 실패 지점을 피해야 합니다.
    - **Root Token 관리 원칙:** 최초의 Root Token은 초기 설정을 위해서만 사용하고, 이후에는 권한이 제한된 관리자 계정을 사용하며 Root Token은 안전하게 봉인(보관)하거나 폐기하는 것을 원칙으로 합니다.

  - **마스터 키 교체 (Rekey)**
    - `rekey`는 담당자 변경(퇴사 등) 또는 정기 보안 강화를 위한 핵심적인 정기 활동이며, **서비스 중단 없이** 수행 가능합니다.
    - 이 작업은 자동화할 수 없으며, 반드시 **사람(관리자 그룹)이 직접 수행**해야 하는 보안 의식(Security Ceremony)입니다.
    - 모든 관리자가 물리적으로 모일 필요 없이, Signal과 같은 종단간 암호화(E2EE) 통신 채널을 통해 원격으로 안전하게 수행할 수 있습니다.

  - **인증서 교체 시나리오 및 영향 분석**
    - **시나리오 1: 서버/클라이언트 인증서 교체 (일반 유지보수):** **영향 제한적.** 동일한 CA로부터 발급된 새 인증서로 교체하는 경우, 해당 컴포넌트만 업데이트하면 되며 다른 시스템은 영향을 받지 않습니다.
    - **시나리오 2: CA 인증서 교체 (재해 상황):** **영향 시스템 전체.** 신뢰 체인의 최상단인 CA 자체를 교체하는 경우, **EMQX, 서버 리스너, 모든 IoT 장치 및 앱** 등 모든 구성 요소가 새로운 CA를 신뢰하도록 업데이트해야 합니다.

  - **CA 인증서 교체 재해 복구 계획**
    - **기술적 대비:** Flutter 앱은 **API를 통해** 동적으로 최신 CA를 가져오고, 라즈베리파이는 **OTA 업데이트**를 통해 설정 파일을 안전하게 배포받는 방식으로 신속한 업데이트를 대비합니다. (기존 `12. OTA 업데이트 아키텍처`와 연계)
    - **절차적 대비:** CA 교체 시나리오에 대한 **단계별 계획을 문서화**하고, 테스트 환경에서 **주기적인 훈련**을 수행하여 실제 상황에 대비합니다.

### 3. CRUD 계층에서의 권한 처리 및 명시적 예외 사용 (Authorization in CRUD Layer & Explicit Exceptions)

기존 `server1`의 엔드포인트 레벨에서 `Depends`를 통해 처리되던 권한 검사 로직을 `server2`에서는 CRUD 계층으로 이동시켜 중앙화하고 명확성을 높입니다.

-   **중앙화된 인가 서비스:** `core/authorization.py` 모듈이 모든 권한 확인 로직의 단일 책임 지점(Single Source of Truth) 역할을 합니다.
-   **CRUD 계층의 책임:**
    1.  모든 데이터 조작(C, U, D) 및 민감한 데이터 조회(R) 전에, 인가 서비스를 호출하여 명시적으로 권한을 확인합니다.
    2.  권한이 없거나, 리소스를 찾지 못하는 등 비즈니스 규칙 위반 시, `None`을 반환하는 대신 `PermissionDeniedError`, `NotFoundError`와 같은 명시적인 사용자 정의 예외를 발생시킵니다.
-   **API 계층의 역할:** CRUD 계층에서 발생한 예외를 `try...except`로 포착하여, 사용자에게 친숙한 HTTP 상태 코드(403, 404 등)와 메시지로 변환하여 응답합니다.
-   **기대 효과:** 비즈니스 로직과 API 로직의 관심사를 명확히 분리하고, 코드 재사용성을 높이며, 시스템의 전체적인 안정성과 보안을 강화합니다.

### 4. 관리자 패널의 멀티-테넌트 지원 설계 (Multi-tenancy Support for Admin Panel)

향후 '기업용 패널'로의 확장을 고려하여, 여러 조직의 관리 권한을 가진 사용자를 지원하기 위해 '컨텍스트 전환(Context Switching)' 모델을 채택합니다.

-   **1. 컨텍스트 선택:** 사용자는 로그인 후 관리할 특정 조직(예: 'A회사')을 명시적으로 선택합니다.
-   **2. 스코프 고정:** 선택된 조직의 컨텍스트 내에서만 모든 UI 및 API 호출이 이루어집니다.
-   **3. 명확한 분리:** 다른 조직(예: 'B회사')을 관리하려면, 사용자는 UI를 통해 명시적으로 컨텍스트를 전환해야 합니다.

**구현 방안:**

-   **조직별 권한 검사:** `authorization.py`의 `check_user_permission` 함수는 `organization_id`를 인자로 받아, 사용자가 해당 조직 내에서 특정 작업을 수행할 권한이 있는지 정교하게 검사합니다. (예: A회사 관리자는 B회사 데이터를 조작할 수 없음)
-   **API와 데이터 필터링:** 프론트엔드에서 전달된 `organization_id`를 기반으로, API와 CRUD 계층은 모든 데이터 조회 및 조작이 해당 조직 범위 내에서만 이루어지도록 강제합니다.

### 5. 고도화된 접근 제어 모델 (Advanced Access Control Model)

단순한 역할 기반 접근 제어(RBAC)를 넘어, 멀티-테넌트 SaaS 환경에 필수적인 더 정교하고 유연한 접근 제어 모델을 도입합니다. 이 모델은 소유권, 역할, 데이터의 속성을 모두 고려하여 보안과 사용성을 극대화합니다.

-   **이원화된 권한 모델 (Dual Permission Model):** 시스템은 두 가지 종류의 권한을 가집니다.
    -   **전역 권한 (Global Permissions):** `SYSTEM` 스코프를 가지며, 플랫폼 전체의 운영 및 관리에 필요한 권한입니다. 이 권한은 시스템 관리자(Superuser) 또는 시스템 레벨 역할을 가진 사용자에게 부여됩니다. (향후 2단계에서 `permissions` 테이블에 `organization_id=NULL`로 구현)
    -   **조직별 권한 (Per-Organization Permissions):** `ORGANIZATION` 스코프를 가지며, 특정 조직 내에서 리소스를 관리하는 권한입니다. 각 기업(조직)의 구성원에게 역할(Role)을 통해 부여됩니다. (향후 2단계에서 `permissions` 테이블에 `organization_id=기업ID`로 구현)

-   **장치 소유권 및 접근 모델 (Device Ownership & Access Model):**
    -   **기업 장치 (Corporate Device):** `devices` 테이블의 `organization_id`에 기업 ID가 할당된 장치입니다. 이 장치에 대한 접근은 해당 조직 내에서 적절한 역할을 부여받은 사용자만 가능합니다.
    -   **개인 장치 (Personal Device):** `devices` 테이블의 `organization_id`가 `NULL`인 장치입니다. 이 장치에 대한 접근은 `user_devices` 테이블을 통해 확인된 '소유주(owner)'만 가능합니다.

-   **계층적 인가 확인 (Layered Authorization Checks):** CRUD 계층에서 리소스에 대한 접근을 확인할 때, 다음과 같은 계층적 검사를 수행합니다.
    1.  **소유권 우선 확인:** 먼저 요청자가 해당 리소스의 '소유주'인지 확인하여 접근을 허용합니다. (예: 개인 장치 소유주)
    2.  **역할 기반 권한 확인 (RBAC):** 소유주가 아닐 경우, 요청자가 해당 작업을 수행할 수 있는 '전역 권한' 또는 '조직별 권한'을 가졌는지 `core/authorization.py` 서비스를 통해 확인합니다.

-   **향후 고도화 계획 (Future Enhancements):**
